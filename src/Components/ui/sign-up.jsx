...
export const AuthComponent = ({ 
  ...
  // Show external errors (e.g. from Firebase)
  useEffect(() => {
    if (externalError) {
  ...
  const handleFinalSubmit = async (e) => {
    ...
    // Login mode: just need email + password
    if (isLoginMode) {
  ...
  const handleProgressStep = () => {
    if (isLoginMode) {
      // In login mode, step from email → password, then submit
    ...
    <AnimatePresence>
        {modalStatus !== 'closed' && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
                <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} className="relative bg-[var(--card)]/80 border-4 border-[var(--border)] rounded-2xl p-8 w-full max-w-sm flex flex-col items-center gap-4 mx-2">
                    {(modalStatus === 'error' || modalStatus === 'success') && <button onClick={closeModal} className="absolute top-2 right-2 p-1 text-[var(--muted-foreground)] hover:text-[var(--foreground)] transition-colors"><X className="w-5 h-5" /></button>}
                    {modalStatus === 'error' && <>
                        <AlertCircle className="w-12 h-12 text-red-500" />
                        <p className="text-lg font-medium text-[var(--foreground)]">{modalErrorMessage}</p>
                        ...
                        <TextLoop interval={TEXT_LOOP_INTERVAL} stopOnEnd={true}>
                            {modalSteps.slice(0, -1).map((step, i) => 
                                <div key={i} className="flex flex-col items-center gap-4">
                                    {step.icon}
                                    <p className="text-lg font-medium text-[var(--foreground)]">{step.message}</p>
                                </div>
                            )}
                        </TextLoop>
                    }
                    {modalStatus === 'success' &&
                        <div className="flex flex-col items-center gap-4">
                            ...
                            <p className="text-lg font-medium text-[var(--foreground)]">{modalSteps[modalSteps.length - 1].message}</p>
                        </div>
                    }
                </motion.div>
            </motion.div>
  ...
  const titleText = isLoginMode 
    ...
    : (authStep === 'email' ? 'Get started with Us' : authStep === 'password' ? 'Create your password' : 'One Last Step');

  const subtitleText = isLoginMode
    ? (authStep === 'email' ? 'Sign in to continue your focus flow' : 'Your password for ' + email)
    : (authStep === 'password' ? 'Your password must be at least 6 characters long.' : authStep === 'confirmPassword' ? 'Confirm your password to continue' : 'Continue with');

  return (
    <div className="min-h-screen w-screen flex flex-col" style={{ background: 'var(--background)' }}>
        <style>{`
        ...
        <div className={cn( "fixed top-4 left-4 z-20 flex items-center gap-2", "md:left-1/2 md:-translate-x-1/2" )}>
            {logo}
            <h1 className="text-base font-bold" style={{ color: 'var(--foreground)' }}>{brandName}</h1>
        </div>

        <div className={cn("flex w-full flex-1 h-full items-center justify-center", "relative overflow-hidden")} style={{ background: 'var(--card)' }}>
            ...
            <fieldset disabled={modalStatus !== 'closed'} className="relative z-10 flex flex-col items-center gap-8 w-[280px] mx-auto p-4">
                <AnimatePresence mode="wait">
                    {authStep === "email" && <motion.div key="email-content" initial={{ y: 6, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ opacity: 0 }} transition={{ duration: 0.3, ease: "easeOut" }} className="w-full flex flex-col items-center gap-4">
                        <BlurFade delay={0.25 * 1} className="w-full"><div className="text-center"><p className="font-serif font-light text-4xl sm:text-5xl md:text-6xl tracking-tight whitespace-nowrap" style={{ color: 'var(--foreground)' }}>{titleText}</p></div></BlurFade>
                        <BlurFade delay={0.25 * 2}><p className="text-sm font-medium" style={{ color: 'var(--muted-foreground)' }}>{subtitleText}</p></BlurFade>
                        <BlurFade delay={0.25 * 3}><div className="flex items-center justify-center gap-4 w-full">
                            <GlassButton onClick={handleGoogleSignIn} contentClassName="flex items-center justify-center gap-2" size="sm"><GoogleIcon /><span className="font-semibold" style={{ color: 'var(--foreground)' }}>Google</span></GlassButton>
                        </div></BlurFade>
                        <BlurFade delay={0.25 * 4} className="w-[300px]"><div className="flex items-center w-full gap-2 py-2"><hr className="w-full" style={{ borderColor: 'var(--border)' }}/><span className="text-xs font-semibold" style={{ color: 'var(--muted-foreground)' }}>OR</span><hr className="w-full" style={{ borderColor: 'var(--border)' }}/></div></BlurFade>
                    </motion.div>}
                    {authStep === "password" && !isLoginMode && <motion.div key="password-title" initial={{ y: 6, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ opacity: 0 }} transition={{ duration: 0.3, ease: "easeOut" }} className="w-full flex flex-col items-center text-center gap-4">
                        <BlurFade delay={0} className="w-full"><div className="text-center"><p className="font-serif font-light text-4xl sm:text-5xl tracking-tight whitespace-nowrap" style={{ color: 'var(--foreground)' }}>{titleText}</p></div></BlurFade>
                        <BlurFade delay={0.25 * 1}><p className="text-sm font-medium" style={{ color: 'var(--muted-foreground)' }}>{subtitleText}</p></BlurFade>
                    </motion.div>}
                ...
                <form onSubmit={handleFinalSubmit} className="w-[300px] space-y-6">
                        ...
                        {authStep !== 'confirmPassword' && <motion.div key="email-password-fields" exit={{ opacity: 0, filter: 'blur(4px)' }} transition={{ duration: 0.3, ease: "easeOut" }} className="w-full space-y-6">
                            <BlurFade delay={authStep === 'email' ? 0.25 * 5 : 0} inView={true} className="w-full">
                                <div className="relative w-full">
                                    <AnimatePresence>
                                        {authStep === "password" && <motion.div initial={{ y: -10, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ duration: 0.3, delay: 0.4 }} className="absolute -top-6 left-4 z-10"><label className="text-xs font-semibold" style={{ color: 'var(--muted-foreground)' }}>Email</label></motion.div>}
                                    </AnimatePresence>
                                    <div className="glass-input-wrap w-full"><div className="glass-input">
                                        <span className="glass-input-text-area"></span>
                                        <div className={cn( "relative z-10 flex-shrink-0 flex items-center justify-center overflow-hidden transition-all duration-300 ease-in-out", email.length > 20 && authStep === 'email' ? "w-0 px-0" : "w-10 pl-2" )}><Mail className="h-5 w-5 flex-shrink-0" style={{ color: 'oklch(from var(--foreground) l c h / 80%)' }}/></div>
                                        <input type="email" placeholder="Email" value={email} onChange={(e) => setEmail(e.target.value)} onKeyDown={handleKeyDown} className={cn("relative z-10 h-full w-0 flex-grow bg-transparent focus:outline-none transition-[padding-right] duration-300 ease-in-out delay-300", isEmailValid && authStep === 'email' ? "pr-2" : "pr-0")} style={{ color: 'var(--foreground)' }} />
                            ...
                            <AnimatePresence>
                                {authStep === "password" && <BlurFade key="password-field" className="w-full">
                                    <div className="relative w-full">
                                        <AnimatePresence>
                                            {password.length > 0 && <motion.div initial={{ y: -10, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ duration: 0.3 }} className="absolute -top-6 left-4 z-10"><label className="text-xs font-semibold" style={{ color: 'var(--muted-foreground)' }}>Password</label></motion.div>}
                                        </AnimatePresence>
                                        <div className="glass-input-wrap w-full"><div className="glass-input">
                                            <span className="glass-input-text-area"></span>
                                            <div className="relative z-10 flex-shrink-0 flex items-center justify-center w-10 pl-2">
                                                {isPasswordValid ? <button type="button" aria-label="Toggle password visibility" onClick={() => setShowPassword(!showPassword)} className="transition-colors p-2 rounded-full" style={{ color: 'oklch(from var(--foreground) l c h / 80%)' }}>{showPassword ? <EyeOff className="w-5 h-5" /> : <Eye className="w-5 h-5" />}</button> : <Lock className="h-5 w-5 flex-shrink-0" style={{ color: 'oklch(from var(--foreground) l c h / 80%)' }} />}
                                            </div>
                                            <input ref={passwordInputRef} type={showPassword ? "text" : "password"} placeholder="Password" value={password} onChange={(e) => setPassword(e.target.value)} onKeyDown={handleKeyDown} className="relative z-10 h-full w-0 flex-grow bg-transparent focus:outline-none" style={{ color: 'var(--foreground)' }} />
                                    ...
                                    <BlurFade inView delay={0.2}><button type="button" onClick={handleGoBack} className="mt-4 flex items-center gap-2 text-sm transition-colors" style={{ color: 'oklch(from var(--foreground) l c h / 70%)' }}><ArrowLeft className="w-4 h-4" /> Go back</button></BlurFade>
                                </BlurFade>}
                            </AnimatePresence>
                        </motion.div>}
                    </AnimatePresence>
                    <AnimatePresence>
                        {authStep === 'confirmPassword' && <BlurFade key="confirm-password-field" className="w-full">
                            <div className="relative w-full">
                                <AnimatePresence>
                                    {confirmPassword.length > 0 && <motion.div initial={{ y: -10, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ duration: 0.3 }} className="absolute -top-6 left-4 z-10"><label className="text-xs font-semibold" style={{ color: 'var(--muted-foreground)' }}>Confirm Password</label></motion.div>}
                                </AnimatePresence>
                                <div className="glass-input-wrap w-[300px]"><div className="glass-input">
                                    <span className="glass-input-text-area"></span>
                                    <div className="relative z-10 flex-shrink-0 flex items-center justify-center w-10 pl-2">
                                        {isConfirmPasswordValid ? <button type="button" aria-label="Toggle confirm password visibility" onClick={() => setShowConfirmPassword(!showConfirmPassword)} className="transition-colors p-2 rounded-full" style={{ color: 'oklch(from var(--foreground) l c h / 80%)' }}>{showConfirmPassword ? <EyeOff className="w-5 h-5" /> : <Eye className="w-5 h-5" />}</button> : <Lock className="h-5 w-5 flex-shrink-0" style={{ color: 'oklch(from var(--foreground) l c h / 80%)' }} />}
                                    </div>
                                    <input ref={confirmPasswordInputRef} type={showConfirmPassword ? "text" : "password"} placeholder="Confirm Password" value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} className="relative z-10 h-full w-0 flex-grow bg-transparent focus:outline-none" style={{ color: 'var(--foreground)' }} />
                ...
                <BlurFade delay={0.25 * 6} className="text-center">
                    <p className="text-sm" style={{ color: 'var(--muted-foreground)' }}>
                        ...
                        <button 
                            ...
                            className="font-semibold ml-2 hover:underline transition-colors"
