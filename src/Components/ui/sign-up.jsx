    ...
    <>
        ...
        <svg width="100%" height="100%" viewBox="0 0 800 600" fill="none" xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="xMidYMid slice" className="absolute top-0 left-0 w-full h-full">
            <defs>
                ...
                <linearGradient id="rev_grad2" x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" style={{stopColor: 'var(--color-indigo-500)', stopOpacity:0.7}} /><stop offset="50%" style={{stopColor: 'var(--color-emerald-500)', stopOpacity:0.5}} /><stop offset="100%" style={{stopColor: 'var(--color-emerald-600)', stopOpacity:0.4}} /></linearGradient>
...
export const AuthComponent = ({ 
        ...
        {modalStatus !== 'closed' && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-md">
                <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} className="relative bg-[var(--color-slate-900)] border-4 border-[var(--color-slate-700)] rounded-3xl p-8 w-full max-w-sm flex flex-col items-center gap-4 mx-2 shadow-2xl">
                    {(modalStatus === 'error' || modalStatus === 'success') && <button onClick={closeModal} className="absolute top-4 right-4 p-1 text-[var(--color-slate-400)] hover:text-[var(--color-white)] transition-colors"><X className="w-5 h-5" /></button>}
                    {modalStatus === 'error' && <>
  ...
  return (
    <div className="min-h-screen w-screen flex flex-col" style={{ background: 'var(--color-slate-950)' }}>
        <style>{`
            ...
            <fieldset disabled={modalStatus !== 'closed'} className="relative z-10 flex flex-col items-center gap-8 w-[280px] mx-auto p-4">
                <AnimatePresence mode="wait">
                    {authStep === "email" && <motion.div key="email-content" initial={{ y: 6, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ opacity: 0 }} transition={{ duration: 0.3, ease: "easeOut" }} className="w-full flex flex-col items-center gap-4">
                        <BlurFade delay={0.25 * 1} className="w-full"><div className="text-center"><p className="font-serif font-light text-4xl sm:text-5xl md:text-6xl tracking-tight whitespace-nowrap" style={{ color: 'var(--color-white)' }}>{titleText}</p></div></BlurFade>
                        <BlurFade delay={0.25 * 2}><p className="text-sm font-bold opacity-80" style={{ color: 'var(--color-slate-400)' }}>{subtitleText}</p></BlurFade>
                        <BlurFade delay={0.25 * 3}><div className="flex items-center justify-center gap-4 w-full">
                            <GlassButton onClick={handleGoogleSignIn} contentClassName="flex items-center justify-center gap-2" size="sm"><GoogleIcon /><span className="font-bold" style={{ color: 'var(--color-white)' }}>Google</span></GlassButton>
                        </div></BlurFade>
                        <BlurFade delay={0.25 * 4} className="w-[300px]"><div className="flex items-center w-full gap-2 py-2"><hr className="w-full" style={{ borderColor: 'var(--color-slate-700)' }}/><span className="text-xs font-bold opacity-60" style={{ color: 'var(--color-slate-400)' }}>OR</span><hr className="w-full" style={{ borderColor: 'var(--color-slate-700)' }}/></div></BlurFade>
                    </motion.div>}
                ...
                <BlurFade delay={0.25 * 6} className="text-center">
                    <p className="text-sm font-medium" style={{ color: 'var(--color-slate-400)' }}>
                        {isLoginMode ? "Don't have an account?" : "Already have an account?"}
                        <button 
                            type="button"
                            onClick={onToggleMode} 
                            className="font-bold ml-2 hover:text-[var(--color-emerald-500)] underline transition-colors"
