        ...
        <div className="flex items-center gap-4 flex-1 min-w-0">
          {hasTopics && (
            <motion.button 
              ...
              whileTap={{ scale: 0.9 }}
            >
              <FaChevronRight className={`text-[12px] text-[var(--color-slate-400)] transition-transform duration-300 ${isExpanded ? 'rotate-90' : ''}`} />
            </motion.button>
          )}
          ...
          <div className="flex-1 min-w-0">
            <h3 className="text-[16px] font-semibold text-white tracking-tight truncate">{project.name}</h3>
            <p className="text-[13px] text-[var(--color-slate-400)] font-medium mt-0.5">{formatTime(projectTime)}</p>
          </div>
        </div>
