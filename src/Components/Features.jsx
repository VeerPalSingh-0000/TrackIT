  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-[var(--color-slate-950)]/80 backdrop-blur-md">
      <motion.div
        ...
        className="w-full max-w-3xl glass-card-elevated rounded-3xl shadow-[0_8px_32px_rgba(0,0,0,0.5)] border border-white/10 relative mx-4 max-h-[90dvh] flex flex-col overflow-hidden transition-colors"
      >
        <div className="flex-shrink-0 p-6 sm:p-8 pb-4 sm:pb-6 relative border-b border-white/10 bg-white/[0.02]">
          <div className="absolute top-4 right-4 z-10">
            <button onClick={onClose} className="p-2 text-[var(--color-slate-400)] hover:text-white transition-colors bg-white/5 hover:bg-white/10 rounded-full">
              <FaTimes />
            </button>
          </div>
        ...
        {/* Scrollable content */}
        <div className="flex-1 overflow-y-auto px-6 sm:px-8 pb-6 sm:pb-8 pt-6 bg-white/[0.01]">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 relative z-10">
            {featuresList.map((feature, idx) => (
              <motion.div 
                ...
                className={`${feature.bg} border p-4 sm:p-5 rounded-2xl transition-all duration-300 group hover:scale-[1.02] shadow-inner`}
              >
                <div className={`${feature.color} text-2xl sm:text-3xl mb-3 group-hover:scale-110 transition-transform inline-block`}>
                  {feature.icon}
                </div>
                 <h3 className="text-base sm:text-lg font-bold text-[var(--color-white)] mb-1.5 transition-colors tracking-tight">
                  {feature.title}
                </h3>
                <p className="text-[var(--color-slate-400)] text-sm leading-relaxed transition-colors font-medium">
                  {feature.desc}
                </p>
              </motion.div>
            ))}
