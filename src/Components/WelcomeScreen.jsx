...
import { FaClock, FaTasks, FaMusic, FaChartLine, FaArrowRight, FaArrowLeft, FaRocket } from 'react-icons/fa';
...
const steps = [
  {
    ...
    subtitle: 'Your personal productivity companion for deep work and focused study sessions.',
    visual: 'logo',
  ...
  },
  {
    ...
    subtitle: 'Choose between a free-running Stopwatch or structured Pomodoro cycles (25 min focus / 5 min breaks). The system automatically tracks your sessions.',
    icon: <FaClock />,
  ...
  },
  {
    ...
    subtitle: 'Organize your work into Projects → Topics → Sub-Topics. Track time at every level and know exactly where your hours go.',
    icon: <FaTasks />,
  ...
  },
  {
    ...
    subtitle: 'Block distractions with curated ambient audio — Rain, Forest, Café, or Lo-fi beats. Mix and match the perfect concentration environment.',
    icon: <FaMusic />,
  ...
  },
  {
    ...
    subtitle: 'Review your focus sessions with rich history views. See your daily, weekly, and per-project study patterns to continuously improve.',
    icon: <FaChartLine />,
...
const accentColors = {
  emerald: { dot: 'bg-emerald-400', ring: 'ring-emerald-400/30', text: 'text-emerald-400', iconBg: 'bg-emerald-500/10 border-emerald-500/20', btn: 'bg-emerald-600 hover:bg-emerald-500' },
  violet: { dot: 'bg-violet-400', ring: 'ring-violet-400/30', text: 'text-violet-400', iconBg: 'bg-violet-500/10 border-violet-500/20', btn: 'bg-violet-600 hover:bg-violet-500' },
  blue: { dot: 'bg-blue-400', ring: 'ring-blue-400/30', text: 'text-blue-400', iconBg: 'bg-blue-500/10 border-blue-500/20', btn: 'bg-blue-600 hover:bg-blue-500' },
  amber: { dot: 'bg-amber-400', ring: 'ring-amber-400/30', text: 'text-amber-400', iconBg: 'bg-amber-500/10 border-amber-500/20', btn: 'bg-amber-600 hover:bg-amber-500' },
  rose: { dot: 'bg-rose-400', ring: 'ring-rose-400/30', text: 'text-rose-400', iconBg: 'bg-rose-500/10 border-rose-500/20', btn: 'bg-rose-600 hover:bg-rose-500' },
};
        ...
        <div className="flex-1 flex items-center w-full overflow-hidden">
          <AnimatePresence mode="wait" custom={direction}>
            ...
            >
              ...
              >
                {step.visual === 'logo' ? (
                  <div className={`w-28 h-28 sm:w-32 sm:h-32 rounded-3xl ${colors.iconBg} border flex items-center justify-center shadow-2xl`}>
                    <img src={TrackerLogo} alt="FocusFlow" className="w-20 h-20 sm:w-24 sm:h-24 object-contain drop-shadow-lg" />
                  </div>
                ) : (
                  <div className={`w-20 h-20 sm:w-24 sm:h-24 rounded-3xl ${colors.iconBg} border flex items-center justify-center shadow-2xl`}>
                    <span className={`text-3xl sm:text-4xl ${colors.text}`}>{step.icon}</span>
                  </div>
                )}
              </motion.div>
