...
const StudyTracker = () => {
  ...
  const { 
    timers, topicTimers, subTopicTimers, studyHistory, setStudyHistory, 
    saveSession, formatTime, clearCues 
  } = useSessionManager(
    currentUser, selectedProject, selectedTopic, selectedSubTopic, 
    stopwatch, pomodoro, pomodoroCycle, setPomodoroCycle, POMODORO_DURATIONS
  );
  ...
  useEffect(() => {
    ...
    const unsubscribe = subscribeToUserProjects(currentUser.uid, (fetchedProjects) => {
      ...
      if (fetchedProjects.length > 0) {
        ...
        if (!matchedProject) { 
          matchedProject = fetchedProjects[0]; setSelectedTopic(null); setSelectedSubTopic(null); 
        } else if (currTopic) {
  ...
  const handleCreateOrUpdateProject = useCallback(async (projectData, projectId) => {
    ...
    finally { setActiveModal(null); setEditingProject(null); }
  }, [currentUser]);
  ...
  const deleteProject = useCallback(async (projectId) => {
    ...
    try {
        ...
        if (selectedProject?.id === projectId) {
            ...
            stopwatch.resetTimer(); pomodoro.resetTimer('work'); clearCues();
        }
        toast.success("Project deleted.");
    } catch(error) { toast.error(`Failed to delete: ${error.message}`); }
  }, [projects, selectedProject, stopwatch, pomodoro, setSelectedProject, setSelectedTopic, setSelectedSubTopic, setStudyHistory, clearCues]);

  const handleStartPause = useCallback(() => {
    if (!selectedProject) { toast.error("Please select a task first!"); setActiveModal('selection'); return; }
    if (timerMode === 'stopwatch') { 
      stopwatch.isSessionRunning ? stopwatch.pauseTimer() : stopwatch.startTimer(); 
    } else { 
      if (pomodoro.isActive) { pomodoro.pauseTimer(); } 
      else { pomodoro.startTimer(); if (pomodoro.mode === 'work') { clearCues(); audioService.play('start'); } } 
    }
  ...
  }, [timerMode, stopwatch, pomodoro, saveSession, setPomodoroCycle, clearCues]);
  ...
  const handleSelection = useCallback((project, topic = null, subTopic = null) => {
    ...
    stopwatch.resetTimer(); pomodoro.resetTimer('work'); clearCues(); setActiveModal(null);
  }, [timerMode, stopwatch, pomodoro, handleStopOrReset, setSelectedProject, setSelectedTopic, setSelectedSubTopic, clearCues]);
  ...
  return (
    <>
      ...
      <div className="min-h-screen bg-[var(--color-slate-950)] text-[var(--color-slate-300)] flex flex-col relative overflow-hidden transition-colors duration-500">
        ...
        <Navbar 
          onNewProjectClick={() => { setEditingProject(null); setActiveModal('project'); }} 
        ...
        <AnimatePresence>
          <Suspense fallback={<LoadingScreen message="" overlay={true} />}>
            {activeModal === 'selection' && <SelectionModal projects={projects} onSelect={handleSelection} onDeleteProject={deleteProject} onEditProject={(p) => { setEditingProject(p); setActiveModal('project'); }} timers={timers} topicTimers={topicTimers} subTopicTimers={subTopicTimers} formatTime={formatTime} onClose={() => setActiveModal(null)} />}
            {activeModal === 'history' && <HistoryView projects={projects} studyHistory={studyHistory} formatTime={formatTime} timers={timers} topicTimers={topicTimers} subTopicTimers={subTopicTimers} onClose={() => setActiveModal(null)} />}
            {activeModal === 'project' && <OnboardingFlow onFinish={handleCreateOrUpdateProject} onCancel={() => { setActiveModal(null); setEditingProject(null); }} projectToEdit={editingProject} />}
            {activeModal === 'about' && <About onClose={() => setActiveModal(null)} />}
            {activeModal === 'features' && <Features onClose={() => setActiveModal(null)} />}
          </Suspense>
        </AnimatePresence>
      </div>
    </>
